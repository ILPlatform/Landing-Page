    ...
    callFunction("camps_e_get")().then((response) => {
      ...
      const groupedCamps = camps?.Opportunities__r
        ? Object.values(
          ...
          ).map((campGroup) => ({ ...campGroup, Ages: campGroup?.Ages[0] === 100 ? "???" : campGroup?.Ages.join("-") }))
        : [];
          ...
          <>
            <Container className="text-center">
              ...
              <h2 className="mt-0 h3">
                {formatDate(camps?.Start_Date__c)} - {formatDate(camps?.End_Date__c)}, {camps?.Number_of_Days__c}{" "}
                {data["days"]}{" "}
                          ...
                          <div>
                            <div className="text-left">
                              <h3 className="h5 mx-3 mt-2">
                                ...
                                <i>
                                  {data?.sub?.with} {campGroup?.Parent_Organisation_Name__c}
                                </i>
                                <ul className="mt-2 ml-0 pl-4">
                                  <li>
                                    <b className="bolder">{data?.sub?.age}:</b> {campGroup?.Ages} {data?.sub?.years}
                                  </li>
                                  {/* <li><b className="bolder">{data?.sub?.partner}</b>:  </li> */}
                                  <li>
                                    <b className="bolder">{data?.sub?.address}</b>: {campGroup?.Account?.Name} (
                          ...
                          {campGroup?.Opportunities?.length > 1 ? (
                            <div className="row justify-content-end p-2">
                              {campGroup?.Opportunities?.map((camp) => (
                                <a className="col" href={camp?.Registration_Link__c} target="_blank" rel="noreferrer">
                                  <Button className="justify-content-center w-100">
                                    {camp?.Ages_Announced__c} {data?.sub?.years}, {camp?.Price__c} €
                                  </Button>
                                </a>
                              ))}
                            </div>
                            ...
                            >
                              <Button className="justify-content-center mx-auto w-100">
                                {data?.sub?.register} ({campGroup?.Opportunities[0]?.Price__c} €)
                              </Button>
                            </a>
                          )}
                        </div>
